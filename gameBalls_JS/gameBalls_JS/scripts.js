...
const maxCircles = 15;
const circles = [];
const gravity = 1000; // Earth's gravity 
const dampening = 0.5;
const circlePool = [];
...
function spawnCircle(x, y)  {
    ...
    if (circlePool.length > 1) {
        ...
        if(getcircle()){
            ...
            circlePool.pop
            circlePool.splice(0,circle = new Circle(x, y, radius, color));
        }
...
function animate() {
    ...
    if (circles.length > maxCircles) {
        spliceed = circles.splice(0, circles.length - maxCircles);
        updateInfo();
